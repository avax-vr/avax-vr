...
export function loadTransactionsIntoState() {
  return function thunk(dispatch) {
    let blockchainWS = new WebSocket('wss://avax-ws.lirax.app:8443')
    blockchainWS.onmessage = (msg) => {
      ...
      let newTransaction = JSON.parse(msg.data)
      ...
      return fetch('https://wallet.avax.network/api/price')
        .then((resp) => resp.json())
        .then((msg) => {
          newTransaction.exchangeRate = msg.usd
  ...
  switch (action.type) {
    case ADD_NEW_TRANSACTION: {
      ...
      for (var i = 0; i < newTransaction.outputs.length; i++) {
        amount += parseFloat(newTransaction.outputs[i].readable_amount);
      }
      transactionSize = amount
      transactionUSD = +(newTransaction.exchangeRate * amount).toFixed(2);
