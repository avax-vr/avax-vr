...
function BlockchainInfo(props) {
  ...
  const curCurrency = 'AVAX'
  const { largest, total, last } = props;
  const roundAVAX = 3; // will round to 3 decimal places
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.container_numberValues}>
        <Text style={styles.text_numberValues}>{`${curCurrency} ${formatNumber(total[0], roundAVAX)}`}</Text>
        <Text style={styles.text_numberValues}>{`${curCurrency} ${formatNumber(largest[0], roundAVAX)}`}</Text>
        <Text style={styles.text_numberValues}>{`${curCurrency} ${formatNumber(last[0], roundAVAX)}`}</Text>
      </View>
